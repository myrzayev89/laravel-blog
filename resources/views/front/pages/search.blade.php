...
@section('title', 'Axtarış'.' - '.$q)
@section('content')
    <div class="page-title db">
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                    <h2>Axtarılan Sorğu - {{ $q }}</h2>
                </div><!-- end col -->
                ...
                <div class="col-lg-8 col-md-12 col-sm-12 col-xs-12">
                                ...
                                @foreach($posts as $post)
                                    <div class="blog-box wow fadeIn">
                                        ...
                                        <div class="blog-meta big-meta text-center">
                                            ...
                                            <h4><a href="{{ route('front.article', ['slug' => $post->slug]) }}"
                                                   title="">{{ $post->name }}</a></h4>
                                            <p>{!! $post->desc !!}</p>
                                            <small><a href="{{ route('front.category', ['slug' => $post->category->slug]) }}"
                            ...
                            <div class="col-md-12">
                                <nav aria-label="Page navigation">
                                    <ul class="pagination justify-content-center">
                                        {{ $posts->appends(['q' => request()->q])->links('vendor.pagination.bootstrap-4') }}
                                    </ul>
                                </nav>
                            </div>
                        </div>
                    @else
                        <div class="alert alert-info" style="font-size: 20px;">
                            <i class="fa fa-info-circle"></i> Sorğu Tapılmadı
                        </div>
                    @endif
                </div>
